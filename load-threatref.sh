...
EVENTS = [
    ...
    ("Microsoft-Windows-Windows Defender/Operational", 1006, "Malware detected",
     "Execution", "Malware Detection",
     "Post-execution malware artifact; incomplete payload drop; correlate process/network events around detection time",
     "T1204", "High", 1),
    ("Microsoft-Windows-Windows Defender/Operational", 1007, "Malware action taken",
     "Execution", "Malware Detection",
     "Malware quarantine or remediation; active infection being addressed; track 1008 follow-on for persistent malware",
     "T1204", "High", 1),
    ("Microsoft-Windows-Windows Defender/Operational", 1008, "Malware action failed",
     "Execution,Persistence", "Malware Detection",
     "Persistent malware resisting remediation; rootkit protection mechanism; incomplete quarantine",
     "T1014,T1562", "Critical", 1),
    ("Microsoft-Windows-Windows Defender/Operational", 1013, "Malware detection history deleted",
     "Defense Evasion", "AV Tamper",
     "Anti-forensics; attacker clearing Defender detection history to remove indicators",
     "T1070", "High", 1),
    ("Microsoft-Windows-Windows Defender/Operational", 1116, "Antimalware platform disabled",
     "Defense Evasion", "AV Tamper",
     "Defender disabled before payload execution; high-confidence defense evasion pivot point",
     "T1562.001", "Critical", 1),
    ("Microsoft-Windows-Windows Defender/Operational", 1117, "Antimalware protection removed",
     "Defense Evasion", "AV Tamper",
     "Defender protection removed; service tamper; high-value pre-execution indicator",
     "T1562.001", "Critical", 1),
    ("Microsoft-Windows-Windows Defender/Operational", 5001, "Real-time protection disabled",
     "Defense Evasion", "AV Tamper",
     "RTP disabled via UI, policy, or attacker command; opens window for undetected payload execution",
     "T1562.001", "Critical", 1),
    ("Microsoft-Windows-Windows Defender/Operational", 5004, "Real-time protection config changed",
     "Defense Evasion", "AV Tamper",
     "RTP configuration weakened; scan sensitivity reduced; exclusion or behavior change applied",
     "T1562.001", "High", 1),
    ("Microsoft-Windows-Windows Defender/Operational", 5007, "Antimalware config changed",
     "Defense Evasion", "AV Tamper",
     "Exclusion path/process/extension added (Set-MpPreference pattern); opens path for undetected execution",
     "T1562.001", "Critical", 1),
    ("Microsoft-Windows-Windows Defender/Operational", 5010, "Scanning for malware disabled",
     "Defense Evasion", "AV Tamper",
     "Scheduled or on-access malware scanning disabled; coverage reduction before payload staging",
     "T1562.001", "High", 1),
    ("Microsoft-Windows-Windows Defender/Operational", 5012, "Virus scanning disabled",
     "Defense Evasion", "AV Tamper",
     "Real-time virus scanning disabled; file-based malware execution window opened",
     "T1562.001", "High", 1),

    # ── RDP LocalSessionManager ───────────────────────────────────────────────
    ("Microsoft-Windows-TerminalServices-LocalSessionManager/Operational", 21, "RDP session logon succeeded",
     "Lateral Movement", "RDP",
     "Remote interactive logon; lateral movement confirmation; admin session establishment after credential abuse",
     "T1021.001,T1078", "High", 2),
    ("Microsoft-Windows-TerminalServices-LocalSessionManager/Operational", 23, "RDP session logoff",
     "Lateral Movement", "RDP",
     "Session duration tracking; short-lived admin sessions indicate tooling; logon-action-logoff operator pattern",
     "T1021.001,T1078", "Low", 2),
    ("Microsoft-Windows-TerminalServices-LocalSessionManager/Operational", 24, "RDP session disconnected",
     "Lateral Movement", "RDP",
     "Session windowing; potential session hijack precursor; operator movement tracking",
     "T1021.001,T1078", "Medium", 2),
    ("Microsoft-Windows-TerminalServices-LocalSessionManager/Operational", 25, "RDP session reconnected",
     "Lateral Movement", "RDP",
     "Session hijack; reconnect after credential abuse; admin session reuse by attacker",
     "T1021.001,T1078", "High", 2),
    ("Microsoft-Windows-TerminalServices-LocalSessionManager/Operational", 39, "Session disconnect initiated",
     "Lateral Movement,Defense Evasion", "RDP",
     "Operator-initiated disconnect; covering tracks; overlapping session activity pattern",
     "T1021.001", "Medium", 2),
    ...
    ("Microsoft-Windows-TerminalServices-RemoteConnectionManager/Operational", 1149,
     ...
     "Lateral Movement", "RDP",
     "Network-level authentication passed; inbound RDP session established; lateral movement",
     "T1021.001,T1078", "High", 2),

    # ── RDP RdpCoreTS (source IP attribution) ─────────────────────────────────
    ("Microsoft-Windows-RemoteDesktopServices-RdpCoreTS/Operational", 131,
     "RDP TCP connection received",
     "Lateral Movement,Credential Access", "RDP",
     "Source IP attribution for inbound RDP; brute force event correlation; RDP scan detection",
     "T1021.001,T1110", "High", 2),
    ("Microsoft-Windows-RemoteDesktopServices-RdpCoreTS/Operational", 140,
     "RDP connection failed",
     "Credential Access", "RDP",
     "Failed RDP attempt with source IP; brute force / password spray; NLA-level auth failure",
     "T1110,T1021.001", "High", 2),

    # ── RDP Client (outbound RDP from this host) ──────────────────────────────
    ("Microsoft-Windows-TerminalServices-RDPClient/Operational", 1024,
     "RDP client connection initiated",
     "Lateral Movement", "RDP",
     "Outbound RDP from this host; compromised host used as pivot; stolen admin workstation lateral movement",
     "T1021.001", "High", 2),
    ("Microsoft-Windows-TerminalServices-RDPClient/Operational", 1025,
     "RDP client connection succeeded",
     "Lateral Movement", "RDP",
     "Successful outbound RDP from compromised host; pivot to DC or Tier-0 target confirmed",
     "T1021.001", "High", 2),

    # ── BITS Client Operational ───────────────────────────────────────────────
    ("Microsoft-Windows-Bits-Client/Operational", 3, "BITS job created",
     "Command and Control,Defense Evasion", "BITS",
     "Malware staging via BITS; stealth C2 payload download bypassing proxy/AV using trusted service",
     "T1197,T1105", "High", 2),
    ("Microsoft-Windows-Bits-Client/Operational", 59, "BITS job transfer notification",
     "Command and Control", "BITS",
     "BITS payload in-flight; C2 or dropper download actively occurring via background service",
     "T1197,T1105", "High", 2),
    ("Microsoft-Windows-Bits-Client/Operational", 60, "BITS job completed",
     "Command and Control,Execution", "BITS",
     "BITS download complete; staged payload ready for execution; persistent C2 poll cycle complete",
     "T1197,T1105", "High", 2),
    ("Microsoft-Windows-Bits-Client/Operational", 4, "BITS job error",
     "Command and Control", "BITS",
     "Failed BITS C2 or staging attempt; repeated errors indicate persistent beacon to unreachable C2",
     "T1197", "Medium", 2),

    # ── Print Service Operational ─────────────────────────────────────────────
    ("Microsoft-Windows-PrintService/Operational", 808, "Print spooler plug-in/DLL loaded",
     "Privilege Escalation,Execution", "Print Spooler",
     "Print Nightmare (CVE-2021-34527); malicious DLL loaded by spooler process for LPE or RCE",
     "T1068,T1547", "Critical", 2),
    ("Microsoft-Windows-PrintService/Operational", 316, "Printer driver installed",
     "Privilege Escalation,Execution", "Print Spooler",
     "Print Nightmare exploitation path; unauthorized printer driver deployed via spooler",
     "T1068,T1547", "Critical", 2),

    # ── Windows Update Client Operational ─────────────────────────────────────
    ("Microsoft-Windows-WindowsUpdateClient/Operational", 20, "Update installed successfully",
     "Defense Evasion", "Update Client",
     "Software change via update; malware concealed in servicing package; patch installation context",
     "T1195", "Low", 3),
    ("Microsoft-Windows-WindowsUpdateClient/Operational", 25, "Update download started",
     "Defense Evasion", "Update Client",
     "Large download via update service; potential supply chain or update abuse",
     "T1195", "Low", 3),
    ("Microsoft-Windows-WindowsUpdateClient/Operational", 31, "Update failed to install",
     "Impact,Defense Evasion", "Update Client",
     "Patch suppression; update service disruption preserving unpatched vulnerability; deliberate sabotage",
     "T1195", "Medium", 3),
]
